      ...
      {cartItems.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-pcd-border rounded-xl p-5 h-fit">
            ...
            <Link href="/checkout?order=1" onClick={() => {
                try { sessionStorage.setItem('checkout_order', JSON.stringify({ items: cartItems.map((i: any) => ({ _id: i._id, slug: i.slug, name: i.name, price: i.price, quantity: i.quantity })), form: { name: '', email: '', phone: '', gstin: '', address: '' } })); } catch {}
